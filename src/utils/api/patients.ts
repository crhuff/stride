...
const getPatient = async (patientId: string): Promise<Patient> => {
  try {
    return await getApiData<undefined, Patient>(`patients/${patientId}`, 'GET', undefined);
  } catch (err) {
    // replace with logger in the future
...
const updatePatient = async (patientId: string, patient: Patient): Promise<unknown> => {
  try {
    return await getApiData<Patient, unknown>(`patients/${patientId}`, 'PUT', patient);
  } catch (err) {
    // replace with logger in the future
...
const deletePatient = async (patientId: string): Promise<unknown> => {
  try {
    return await getApiData<undefined, unknown>(`patients/${patientId}`, 'DELETE', undefined);
  } catch (err) {
    // replace with logger in the future
